...
void Port_Init( const Port_ConfigType* ConfigPtr )
{
	...
	if (NULL_PTR == ConfigPtr)
	{
		...
		volatile uint32 delay = 0;
                for(Port_PinType counter = 0 ; counter < PORT_CONFIGURED_PINS ; counter ++)
                {
		...
		delay = SYSCTL_REGCGC2_REG;
                ...
                if( ((Port_ConfigPtr->pin[counter].port_num == 3) && (Port_ConfigPtr->pin[counter].pin_num == 7)) || ((Port_ConfigPtr->pin[counter].port_num == 5) && (Port_ConfigPtr->pin[counter].pin_num == 0)) ) /* PD7 or PF0 */
                {
                    *(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_LOCK_REG_OFFSET) = 0x4C4F434B;                     /* Unlock the GPIOCR register */   
                    SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_COMMIT_REG_OFFSET) ,  Port_ConfigPtr->pin[counter].pin_num);  /* Set the corresponding bit in GPIOCR register to allow changes on this pin */
                 }
                 else if( (Port_ConfigPtr->pin[counter].port_num == 2) && (Port_ConfigPtr->pin[counter].pin_num <= 3) ) /* PC0 to PC3 */
                 {
                ...
                if (Port_ConfigPtr->pin[counter].pin_initial_mode == PORT_PIN_MODE_0)
		{
			/* Clear the corresponding bit in the GPIOAMSEL register to disable analog functionality on this pin */
			CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ANALOG_MODE_SEL_REG_OFFSET) , Port_ConfigPtr->pin[counter].pin_num);

			/* Disable Alternative function for this pin by clear the corresponding bit in GPIOAFSEL register */
			CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ALT_FUNC_REG_OFFSET) , Port_ConfigPtr->pin[counter].pin_num);

			/* Clear the PMCx bits for this pin */
			*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_CTL_REG_OFFSET) &= ~(0x0000000F << (Port_ConfigPtr->pin[counter].pin_num * 4));

			/* Set the corresponding bit in the GPIODEN register to enable digital functionality on this pin */
			SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIGITAL_ENABLE_REG_OFFSET) , Port_ConfigPtr->pin[counter].pin_num);
		}
		else if (Port_ConfigPtr->pin[counter].pin_initial_mode == PORT_PIN_MODE_ADC)
		{
			/* Clear the corresponding bit in the GPIODEN register to disable digital functionality on this pin */
			CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIGITAL_ENABLE_REG_OFFSET) , Port_ConfigPtr->pin[counter].pin_num);
			...
			*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_CTL_REG_OFFSET) &= ~(0x0000000F << (Port_ConfigPtr->pin[counter].pin_num * 4));
			...
			SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ANALOG_MODE_SEL_REG_OFFSET) , Port_ConfigPtr->pin[counter].pin_num);
		}
		else /* Another mode */
		{
			/* Clear the corresponding bit in the GPIOAMSEL register to disable analog functionality on this pin */
			...
			SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_ALT_FUNC_REG_OFFSET) , Port_ConfigPtr->pin[counter].pin_num);

			/* Set the PMCx bits for this pin */
			*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_CTL_REG_OFFSET) |= (Port_ConfigPtr->pin[counter].pin_initial_mode & 0x0000000F << (Port_ConfigPtr->pin[counter].pin_num * 4));
		...
		SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIR_REG_OFFSET) , Port_ConfigPtr->pin[counter].pin_num);

                  if(Port_ConfigPtr->pin[counter].initial_value == PORT_PIN_LEVEL_HIGH)
                  {
                    /* Set the corresponding bit in the GPIODATA register to provide initial value 1 */
                    SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DATA_REG_OFFSET) , Port_ConfigPtr->pin[counter].pin_num);
                   }
		   else
		   {
		     /* Clear the corresponding bit in the GPIODATA register to provide initial value 0 */
		     CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DATA_REG_OFFSET) , Port_ConfigPtr->pin[counter].pin_num);
		   }
		}
		else if(Port_ConfigPtr->pin[counter].direction == PORT_PIN_IN)
		{
		/* Clear the corresponding bit in the GPIODIR register to configure it as input pin */
		CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_DIR_REG_OFFSET) , Port_ConfigPtr->pin[counter].pin_num);
		...
		if(Port_ConfigPtr->pin[counter].resistor == PULL_UP)
		{
			...
			SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_PULL_UP_REG_OFFSET) , Port_ConfigPtr->pin[counter].pin_num);
		}
		else if(Port_ConfigPtr->pin[counter].resistor == PULL_DOWN)
		{
			...
			SET_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_PULL_DOWN_REG_OFFSET) , Port_ConfigPtr->pin[counter].pin_num);
		}
		else
		{
			/* Clear the corresponding bit in the GPIOPUR register to disable the internal pull up pin */
			CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_PULL_UP_REG_OFFSET) , Port_ConfigPtr->pin[counter].pin_num);

			/* Clear the corresponding bit in the GPIOPDR register to disable the internal pull down pin */
			CLEAR_BIT(*(volatile uint32 *)((volatile uint8 *)PortGpio_Ptr + PORT_PULL_DOWN_REG_OFFSET) , Port_ConfigPtr->pin[counter].pin_num);
		}
		}
		else
		{
...
{
  	boolean error = FALSE;
        /* PORT105: If development error detection for the Port Driver module is enabled: In 
           case the function Port_Init is called with a NULL ConfigPtr and if a variant containing 
           postbuild multiple selectable configuration parameters is used (VARIANT-POST-BUILD), the function Port_Init shall raise the development error 
		...
		if(Port_ConfigPtr->pin[Pin].change_direction == PIN_DIRECTION_UNCHANGEABLE)
		{
			Det_ReportError(PORT_MODULE_ID, PORT_INSTANCE_ID, PORT_SET_UP_PIN_DIRECTION_SID, PORT_E_DIRECTION_UNCHANGEABLE);
                        error = TRUE;
		}
		else
		{	/* Do Nothing */	}
        ...
        /* PORT041: The function Port_Init shall initialize ALL ports and port pins with the 
...
************************************************************************************/
  ...
  void Port_RefreshPortDirection( void )
  {
     ...
     for(Port_PinType counter = 0; counter < PORT_CONFIGURED_PINS; counter++)
	{
		...
		if( (Port_ConfigPtr->pin[counter].port_num == 2) && (Port_ConfigPtr->pin[counter].pin_num <= 3)) /* PC0 to PC3 */
		{
		/* Do Nothing ...  this is the JTAG pins */
		continue;
		}
		 ...
		 * refreshing that are configured as ?pin direction changeable during runtime? 
                */
		if (Port_ConfigPtr->pin[counter].change_direction == PIN_DIRECTION_UNCHANGEABLE)
		{
			if(Port_ConfigPtr->pin[counter].direction == PORT_PIN_OUT)
			{
				/* Set the corresponding bit in the GPIODIR register to configure it as output pin */
